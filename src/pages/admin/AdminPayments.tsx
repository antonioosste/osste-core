...
import { CreditCard, Loader2, DollarSign, Gift } from "lucide-react";
  ...
  useEffect(() => {
    async function fetch() {
      const [{ data: billingData }, { data: giftData }] = await Promise.all([
        supabase.from("user_billing").select("*").order("created_at", { ascending: false }).limit(200),
        supabase.from("gift_invitations").select("*").order("created_at", { ascending: false }).limit(200),
      ]);
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">${giftRevenue.toFixed(2)}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{billing.length + gifts.length}</p>
              </div>
                  ...
                  <TableBody>
                    {billing.map((b) => (
                      <TableRow key={b.id}>
                        ...
                        <TableCell>
                          <Badge variant={b.payment_status === "paid" || b.payment_status === "completed" ? "default" : "secondary"} className="text-xs">
                            {b.payment_status}
                          </Badge>
                        </TableCell>
                  ...
                  <TableBody>
                    {gifts.map((g) => (
                      <TableRow key={g.id}>
                        ...
                        <TableCell>
                          <Badge variant={g.status === "redeemed" ? "default" : g.status === "paid" || g.status === "sent" ? "secondary" : "outline"} className="text-xs capitalize">
                            {g.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">
