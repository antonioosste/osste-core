...
export default function AdminStories() {
  ...
  useEffect(() => {
    async function fetch() {
      const { data } = await supabase.from("stories").select("*").order("created_at", { ascending: false }).limit(500);
      if (data) {
        const groupIds = [...new Set(data.map((s: any) => s.story_group_id).filter(Boolean))];
        const { data: groups } = await supabase.from("story_groups").select("id, title, user_id").in("id", groupIds);
        ...
        setStories(data.map((s: any) => {
          ...
          return { ...s, group_title: group?.title || "—", user_name: profileMap.get(group?.user_id)?.name || "Unknown" };
        }));
      }
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="relative">
            ...
            <Input placeholder="Search stories..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" /> Stories ({filtered.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
                  ...
                  <TableBody>
                    {paged.map((story) => (
                      <TableRow key={story.id}>
                        ...
                        <TableCell>
                          <Badge variant={story.approved ? "default" : "secondary"} className="text-xs">{story.approved ? "Approved" : "Draft"}</Badge>
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">{story.created_at ? format(new Date(story.created_at), "MMM d, yyyy") : "—"}</TableCell>
                        <TableCell className="text-right">
                          <Button size="sm" variant="ghost" onClick={() => setSelected(story)}><Eye className="h-3 w-3" /></Button>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-auto">
          <DialogHeader><DialogTitle>{selected?.title || "Untitled Story"}</DialogTitle></DialogHeader>
          {selected && (
