...
export async function createTurn(
  ...
  const response = await fetchWithRetry(`${BACKEND_BASE}/api/turns/upload`, {
    method: "POST",
    headers: {
...
export async function pollForTTS(
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      const response = await fetch(`${BACKEND_BASE}/api/turns/${turnId}/tts`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export async function generateChapters(token: string, sessionId: string) {
  ...
  const response = await fetchWithRetry(`${BACKEND_BASE}/api/ai/chapter/by-session/${sessionId}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
 ...
 */
export async function assembleStory(token: string, sessionId: string, styleInstruction?: string | null) {
  const body = styleInstruction ? { style_instruction: styleInstruction } : undefined;

  const response = await fetchWithRetry(`${BACKEND_BASE}/api/ai/story/assemble/${sessionId}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
...
export async function deleteImageViaBackend(token: string, imageId: string): Promise<void> {
  const response = await fetchWithRetry(`${BACKEND_BASE}/api/story/delete-image`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
